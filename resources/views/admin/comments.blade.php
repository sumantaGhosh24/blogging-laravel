<x-app-layout>
    <x-slot:title>Admin Manage Comments</x-slot>

    <div class="pt-8">
        <h2 class="text-2xl font-bold mb-4 text-center">Manage Comments</h2>
                    ...
                    @forelse ($comments as $comment)
                        <tr>
                            ...
                            <td class="py-3 px-6 text-left">
                                <a href="{{ route('admin.blog.edit', ['id' => $comment->blog_id]) }}" class="w-fit bg-green-500 px-4 py-2 rounded-md hover:bg-green-600 transition-colors disabled:bg-green-200 text-white underline">{{ $comment->blog->title }}</a>
                            </td>
                            <td class="py-3 px-6 text-left">{{ $comment->owner->email }}</td>
